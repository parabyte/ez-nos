...
/* kpk, configured in nos.cfg, now!
#define BP_DEFAULT_TAB "/etc/bootptab" -> Bootptab 
#define BP_DEFAULT_LOG "/spool/log/bootplog" -> Bootplog
#define BP_DEFAULT_DIR "/etc" -> Bootpdir
#define BP_DEFAULT_FILE "/etc/bootpfile" -> Bootpfile
*/

extern char    *Bootptab; /* = BP_DEFAULT_TAB; kpk*/
static FILE    *bootfp;                 /* bootptab fp */
static long     modtime;                /* last modification time of bootptab */
  
extern char    	*Bootplog;  /* = BP_DEFAULT_LOG; kpk*/
...
extern char     *Bootpdir;   /* bootfile homedirectory kpk, homedir -> bootpdir*/
extern char     *Bootpfile;  /* default file to boot kpk, defaultboot -> Defaultfile*/
...
/*kpk,                strcpy (Bootplog, BP_DEFAULT_LOG); */
...
/*kpk,                        strcpy (Bootpdir, BP_DEFAULT_DIR); */
			bp_log ("Bootp home directory set to: '%s'\n", Bootpdir);
        }
        else {
			strcpy (Bootpdir, argv[1]);
			bp_log ("Bootp home directory set to: '%s'\n", Bootpdir);
        }
    }
    else tprintf (usage);
...
/*kpk,            strcpy (defaultboot, BP_DEFAULT_FILE); */
			bp_log ("Bootp default boot file set to:  '%s'\n", Bootpfile);
		else {
			strcpy (Bootpfile, argv[1]);
			bp_log ("Bootp default boot file set to:  '%s'\n", Bootpfile);
		}
	}
	else
...
readtab()
{
    ...
    if (bootfp == 0) {
/*kpk*/     if ((bootfp = fopen(Bootptab, "r")) == NULLFILE) {
