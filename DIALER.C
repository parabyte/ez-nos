...
void *p;
{
    ...
    if (argc>2) val=atol(argv[2]);
    if (Verbose) tprintf("control %s %ld\n", argv[1], val);
    if ( ifp->ioctl == NULL )
        return -1;
...
void *p;
{
    ...
    {
        Depth = -1;
        tputs("dialer: Blocks nested too deeply\n");
        return -1;
    }
...
void *p;
{
    ...
    if (Depth-- == 0)
    {
        tputs("dialer: `end' without `begin'\n");
        return -1;
    }
    return 0;
}
