...
/* kpk */
extern char *Bootpdir;           /* bootfile Homedirectory */
extern char *Bootpfile;          /* default file to boot */
...
/*kpk*/			&& (memcmp(rq->chaddr, hp->haddr, rq->hlen) == 0)) /* changed strncmp to memcmp kpk*/
				break;
...
/*kpk*/		/*	strcpy(file, Bootpfile); */
	...
	path[0] = 0;
	if((Bootpdir != 0) && (*file != 0)) {
/*kpk,		strcpy(path, *Bootpdir); */
        strcat(path, "/");
    }
...
struct host *hp;
{
	static char cookie[5] = {99, 130, 83, 99, 0}; /* move into there, from line 374 */
	int len;
    int mod;
    int i;
    char    *sizep;
