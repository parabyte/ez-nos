...
#endif
#ifdef  AX25
		if(strcmpi(argv[3],"AX25") == 0
#ifdef POLLEDKISS
			|| strcmpi(argv[3],"PKISS") == 0
#endif
        ) {
...
#ifdef  SLIP
	if(strcmpi(ifp->iftype->name,"SLIP") == 0) {
...
#endif
#ifdef  AX25
		if(strcmpi(ifp->iftype->name,"AX25") == 0
...
#endif
#ifdef  PPP
				if(strcmpi(ifp->iftype->name,"PPP") == 0) {
                    ppp_free(ifp);
                } else
#endif
                {
