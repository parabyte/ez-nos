...
/* Comments to self, kpk 
 * Compiled: eznos(www)  
 * Server: DNS/ftp/http
 * Client: ftp/rdate/smtp 
*/
...
#undef NEWS_TO_MAIL    1 /* NNTPS emails per gateway file */
#undef NN_USESTAT              1 /* Try GROUP/STAT cmds if NEWNEWS fails */
#undef NN_INN_COMPAT    1 /* send "mode reader" cmd after connecting to server */
#undef NN_REMOVE_R_LINES       1 /* remove R: lines from incoming email */
#undef NNTP_TIMEOUT    900 /* idle-timeout #secs for nntp client (both versions)*/
#undef NNTPS_TIMEOUT   3600 /* idle-timeout #secs for nntp server */
...
#undef REMOTECLI       1 /* remote UDP kick/exit/reset */
...
#define ENCRYPT			1 /* Encrypted passwd for ftpusers file, kpk*/
...
#undef AGRESSIVE_GCOLLECT 1 /* exit 251 when availmem < 1/4 of 'mem threshold'*/
...
#undef ED              1 /* editor uses Unix ed syntax; OK for remote sysops. ~13KB */
#undef TED              1 /* editor uses TED syntax; local console only */

/* Protocol options */

#undef AX25            1 /* Ax.25 support */
...
#undef AX25SERVER      1 /* Ax.25 server */
#undef NETROMSERVER    1 /* Net/rom server */
#undef TELNETSERVER    1 /* Tcp telnet server */
...
#define FTPDATA_TIMEOUT  1 /* ftp server timeout on recvfile */
#define FTPSERV_XPERM	 1 /* ftp permmision for msdos */
#define WS_FTP_KLUDGE    1 /* ftp server lies to please ws_ftp winsock app */
#undef FTPSERV_MORE    1 /* ftp server supports RNFR, RNTO, MDTM commands */
#undef FINGERSERVER    1 /* Tcp finger server */
#undef POP2SERVER       1 /* POP2 server -- IAB not recommended */
#undef POP3SERVER      1 /* POP3 server -- IAB draft standard */
#undef POP_TIMEOUT     600 /* pop server idle timeout value, in secs (>= 600) */
#undef REMOTESERVER    1 /* Udp remote server */
...
#define HTTP_EXTLOG      1 /* HTTP: Add detailed access logging in "/wwwlogs" dir */
#define CGI_XBM_COUNTER  1 /* HTTP: Add an X-bitmap counter via CGI */
#define CGI_POSTLOG      1 /* HTTP: Add a POST demo via CGI */
...
#undef BBSSESSION      1 /* bbs (same as telnet localhost) */
...
#undef LOOKSESSION     1 /* follow user activity on the bbs */
...
#undef EXPEDITE_BBS_CMD 1 /* Use MD5 and net.rc to autologin console to bbs */
...
#undef NAX25           24      /* Number of axip interfaces (if defined) */
#undef MAXSCC          4       /* Max number of SCC+ESCC chips (< 16) */


/* Hardware driver options */

#undef ASY             1 /* Asynch driver code */
