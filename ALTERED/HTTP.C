...
#define HTTP_INDEX_NAME "index"
#define HTTP_ROOTINDEX_NAME "index"
#else
...
int HttpUsers = 0;
static int Httpmax = 20;
static int Httpsimc = 10;
...
void *p;
{
    ...
    if (rq.version) {
        while (alarm (Httptdisc * 1000L),recvline (s,Inline,HLINELEN) != -1) {
	   alarm (0L);
           rip (Inline);
	   if(!*Inline) {
           ...
           if(!strnicmp(Inline,wHdrs[HDR_LEN],strlen(wHdrs[HDR_LEN]))) {
                ...
                rq.newcheck = strdup(&Inline[strlen(wHdrs[HDR_SINCE])]);
 ...
 */
    ...
    } else if ((fp = fopen(rq.file,READ_BINARY)) == NULLFILE && !access(rq.file,0)) {
        sprintf(Inline,"http://%s:%u/%s/",rq.myname,ports[rq.index].port,rq.url);
	...
	if ((cp = strrchr (rq.file, '.')) != NULLCHAR)	{
        ...
        if(!htmlAlwaysModified || (thetype != T_html && thetype != T_htm)) {
            if (rq.method && rq.newcheck && !isnewer (sb.st_mtime, rq.newcheck)) {
...
#endif
    if (findfirst(dirstring,&ffblk,REGFILE) || !(ffblk.ff_attrib & FA_DIREC)) { /* error */
      free(dirstring);
#ifdef UNIX
...
/*    sendfile(fp,s,IMAGE_TYPE,0,NULL); */
  ...
  *cp = '\0';  /* drop *.* from dirstring */
  ...
  setflush(s,-1); /* we will do our own flushing */
...
authorization(int s, char *buf, struct reqInfo *rq)
{
	...
	sprintf(buf,"%s/%s",Configdir,Accesswww);
