    ...
    while(resp != -1){
        if(resp == 220){
            ...
            if((cp = ftpcli_login(&ftp, sp->name)) == NULLCHAR){
                if (getline(sp,prmt,buf,LINELEN) == -1) break;
                /* Send the command only if the user response
                 * was non-null
                 */
                rip(buf);
                if(*buf) cp=strdup(buf);
                else if(un != NULLCHAR)
                    cp=strdup(un);
                else {
            ...
            if(ftp.password == NULLCHAR){
                /* turn off echo */
                sp->ttystate.echo = 0;
                if (getline(sp,"Password: ",buf,LINELEN) == -1) break;
                tputc('\n');
                /* Turn echo back on */
                sp->ttystate.echo = 1;
                /* Send the command only if the user response
            ...
            if(cp != NULLCHAR && strnicmp(cp+1,System,strlen(System)) == 0){
                ftp.type = IMAGE_TYPE;
                tputs("Defaulting to binary mode transfers\n");
            }
            resp = 200; /* dummy */
#ifdef DES_AUTH
            ...
            if(ftp.password == NULLCHAR){
                if (getline(sp,"Key ? --> ",buf,LINELEN) == -1) break;
                /* Send the command only if the user response
                 * was non-null
                 */
                if(buf[0] != '\n'){
            ...
            if(argc > 2){
                ...
                if (getline(sp,"ftp> ",buf,LINELEN) == -1) break;
...
#endif
    total = recvfile(fp,ftp->data,ftp->type,(ftp->verbose >= V_HASH) ? ftp->verbose : 0,0L);
...
int putr;       /* Flag: 0 if standard put, 1 if put with resume */
{
    ...
    if(putr == 1){          /* Wait for file offset and checksum */
        line=mallocw(40);
        if (recvline(control,line,40) == -1) goto failure;
        starting=atol(line);
