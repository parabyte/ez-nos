...
#ifdef ENCRYPT
/*kpk */
/* MD5 hash, plain-text passwords in ftpusers file, upto 15 letters */
void
usercvt()
{
	FILE *fp,*fptmp;
	char *buf,*pass,*path;
	char hexbuf[16]; /* kpk, uint8 hexbuf[16],digest[16]; */
	int needsit = 0;
	int len,nlen,plen,i;
	...
	while(fgets(buf,BUFSIZ,fp) != NULL){
		...
		/* for(pass=&buf[nlen]; *pass++ = '\0'; pass = skipwhite(pass); */
		for(pass=&buf[nlen];isspace(*pass);pass++) /* kpk */
			;
		if(*pass != '\0' && *pass != '*'
	...
	while(fgets(buf,BUFSIZ,fp) != NULL){
		...
		/* if(path == strpbrk(pass," \t")) */
		if(path = strpbrk(pass," \t")) { /* kpk, extract remainder of line */
			rip(pass);
			*path++ = '\0';
			path = skipwhite(path);
		}
		...
		MD5Update(&md,(unsigned char *)pass,plen);	/* kpk, Hash password */
		...
		for(i=0;i<16;i++)	/* Write hashed password */
			fprintf(fptmp,"%02x",pass[i]);
		fputc(' ',fptmp);
		fputs(path, fptmp); /* Write remainder of line */
		fputc('\n', fptmp);
	}
 ...
 */
int readhex(out,in,size)
char *out;  /*kpk, uint8 *out; */
char *in;
int size;
{
	int c,count;
