    ...
    for(;;) {
        ...
        for(p = permlinks, plp=NULLPERMLINK; p; p = pnext) {
            ...
            if((s = socket(AF_INET,SOCK_STREAM,0)) == -1)
                continue;
            alarm(CONNECT_TIMEOUT);  /* don't backoff too long! */
...
/* not productive:        usflush(cp->fd); /* try to empty the output queue so we can fill it again! */
...
#else
    usprintf(cp->fd,"%s, Uptime: %s, Core: %lu\n",  /*zkd*/
    shortversion,tformat(secclock()),farcoreleft());
#endif
}
...
#endif
                free(temp->topic);
                temp->topic = strdup(topic);  /* could be NUL */
...
/* #ifdef noblocking	N5KNX: VERY BAD TO RISK LOSING LZW DATA...can't decode!
    sockblock(s,SOCK_NOTXBLOCK);   /* prevent backlogs ! */
/* #endif */
    cp = alloc_connection(s);
