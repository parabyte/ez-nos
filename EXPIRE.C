...
void *p;
{
    ...
    set_timer(&Expiretimer,(uint32)atol(argv[1])*MSPHOUR); /* set timer duration */
...
update_nntp_active() {
  ...
  if((old = fopen(Active,READ_TEXT)) == NULLFILE) {
      ...
      goto xit_actv;  /* unlock and return */
  }
...
void *p;
{
    ...
    set_timer(&Oldbidtimer,(uint32)atol(argv[1])*MSPHOUR); /* set timer duration */
    if(argc > 2)
